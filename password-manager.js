...
var keychain = function() {
  ...
  keychain.init = function(password) {
    priv.secrets.salt = random_bitarray(256);
    priv.secrets.hmac_key = KDF(password, priv.secrets.salt);
  ...
  keychain.get = require_init(function(name) {
      ...
      var result = priv.data.kvs[domain_hmac];
  ...
  */
  keychain.set = require_init(function(name, value) {
      ...
      priv.data.kvs[domain_hmac] = value;
    });
  ...
  */
  keychain.remove = require_init(function(name) {
    ...
    if (priv.data.kvs.hasOwnProperty(name)) {
